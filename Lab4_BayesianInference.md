...
## Tutorial
1. Begin by creating a folder within your `EEB462` folder called `LabFour`. Within that folder, create two additional folders called `allNuc` and `nucProt`. If you have not added MrBayes to your path, you will also need to move your MrBayes executable file to the `LabFour` folder.
...
2. Locate the `NEXUS/MrBayes` folder you created during LabOne. Copy `16s-enam-18s-coi-fuseSimp.nex` to `LabFour/allNuc`, and copy `16s-enam-18s-coiProt-fuse.nex` to `LabFour/nucProt`. 

3. Open `16s-enam-18s-coi-fuseSimp.nex` in your text editor. 

4. At the bottom of the file (after the `END;` command, type `begin MRBAYES;` on a new line
...
5. On another line, enter the following command: `log start filename=allNuc_log.txt;`
...
6. Open the `nucPartition.nex.best_scheme.nex` file that you generated as part of [Lab Three](https://github.com/ddecarle/eeb462-2021/blob/main/Lab3_MaximumLikelihood.md) in your text editor. (It should be located in `../LabThree/allNuc`)
...
7. To define the partitions for your analysis, copy all lines beginning with `charset` into your Bayes block.

Once we've defined the partitions, we need to indicate which partitioning scheme we want to use. 

8. On a new line, enter the following commands:
...
9. Examine `nucPartition.nex.best_scheme.nex` in your text editor.

The lines following `charpartition mymodels = ` indicate the best model for each of our partitions. The model itself is indicated by a three-letter code and is usually followed by a series of parameters that account for different types of rate heterogenetiy across sites. Some common parameters are as follows:
...
10. Use the `lset` and `prset` commands to specify the correct model for each partition according to `nucPartition.nex.best_scheme.nex`. Remember that there should be an `lset` command for each partition, but some (or all) partitions may *not* need a `prset` command. All commands should be entered on their own line. 
...
11. Add the following commands to your MrBayes block:
...
13. On a new line in your MrBayes block, enter the following commands:
...
14. End the MrBayes block using the following command: `END;`

15. Save `16s-enam-18s-coi-fuseSimp.nex` and close the file. 
...
16. In your terminal, navigate to `.../LabFour/allNuc`, and start MrBayes using the following command: 
...
17. When MrBayes has started, load your matrix: `exec 16s-enam-18s-coi-fuseSimp.nex;`
...
19. Select "**File > Import Trace File...**" and open the two `.p` files from the allNuc analysis. 
...
20. Click the "**Trace**" button near the top of the screen.
...
21. Open `allNuc_log.txt` in your text editor, and use it to answer the following questions:
...
22. Open the consensus tree in FigTree and re-root it using the appropriate outgroup. 

23. Tick the "**Node Labels**" box and select "**Display > prob**" to show the posterior probability values for each node
...
24. Open `16s-enam-18s-coiProt-fused.nex` in your text editor

25. Generate a MrBayes block to perform the following functions: 
